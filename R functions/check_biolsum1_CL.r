...
if (length(depth_text)>0) {
  ...
  cat(c("\n","\n"))
  print(bsum_original[depth_text,names(bsum_original)],row.names = FALSE)
  cat(c("\n","\n"))
...
# write a mesage if text found in ID column
sink(file=report_file,append=TRUE, split=TRUE)
if (length(idtxt)>0) {
  bsum$flag[idtxt]=1
  cat("\n","\n")
...
if (length(tests)>0) {
  ...
  cat("\n","\n")
   op=askYesNo("Would you like to remove those records now? ")
   if (op==T) {
     bsum=bsum[-tests,]
